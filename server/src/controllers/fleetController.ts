...
export const getFleetCBOM = async (req: Request, res: Response) => {
  try {
    const { machine_id, tenant } = req.query;
    let query = `
      SELECT 
        ...
        a.machine_id, m.hostname, m.os, m.arch,
        COALESCE(m.tenant_name, t.tenant_name, t.name, 'Default Fleet') as "tenantName",
    ...
    if (tenant && tenant !== 'all') {
      values.push(`%${tenant}%`);
      conditions.push(`(m.tenant_name ILIKE $${values.length} OR t.name ILIKE $${values.length})`);
    }
    ...
    const cbom = {
      ...
      metadata: {
        ...
        component: {
          type: "platform",
          name: machine_id && rows.length > 0 ? `Endpoint: ${rows[0].hostname || machine_id}` : "Enterprise Fleet Cryptographic Assets",
          description: "Cryptographic Bill of Materials (CBOM) generated by Option A Native Scanner"
        }
      },
